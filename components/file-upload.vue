<template>
	<view class="image-upload-list">
		<view class="image-upload-Item" v-for="(item,index) in uploadLists" :key="index">
			<view class="image-upload-Item-video" v-if="isVideo(item)">
				<!-- #ifndef APP-PLUS -->
				<app-img v-if="videoPoster" :src="videoPoster" w="160" h="160" @click="previewVideo(getFileUrl(item))" poster-size="30" />
				<video v-else :disabled="false" :controls="false" :src="getFileUrl(item)">
					<cover-view class="image-upload-Item-video-fixed" @click="previewVideo(getFileUrl(item))">
					</cover-view>

					<cover-view class="image-upload-Item-del-cover" v-if="remove && previewVideoSrc==''" @click="imgDel(index)">×</cover-view>

				</video>
				<!-- #endif -->
				<!-- #ifdef APP-PLUS -->
				<view class="image-upload-Item-video-fixed" @click="previewVideo(getFileUrl(item))"></view>
				<!-- <image class="image-upload-Item-video-app-poster" mode="widthFix" :src="appVideoPoster"></image> -->
				<app-img :src="videoPoster" w="160" h="160" poster-size="30" />
				<!-- #endif -->
			</view>

			<app-img v-else :src="item" @click="imgPreview(getFileUrl(item))" />

			<view class="image-upload-Item-del" v-if="remove" @click="imgDel(index)">×</view>
		</view>
		<view class="image-upload-Item image-upload-Item-add app-flex-center" v-if="uploadLists.length<max && add" @click="chooseFile">
...
<script>
	...
	export default {
		...
		props: {
			...
			sourceType: { //选择照片来源 【ps：H5无效】
				type: Array,
				default: () => ['album', 'camera'],
			},
			action: { //上传地址
				type: String,
				default: 'upload/imageUpload.do',
			},
			headers: { //上传的请求头部
				type: Object,
				default: () => {},
			...
			appVideoPoster: { //app端视频展示封面 只对app有效
				type: String,
				default: '/static/image-upload/play.png',
			},
		},
		data() {
			return {
		...
		methods: {
			...
			getFileUrl(item) {
				...
				return `${config.BASE_URL}upload${url}`
			},
				...
				tempFilePaths.forEach((item, index) => {
					uploadImgs.push(new Promise((resolve, reject) => {
						...
						const uploadTask = uni.uploadFile({
							...
							// #endif
							// #ifndef H5
                            url: `${config.BASE_URL}${this.action}`,
							// #endif
							filePath: item,
							name: this.name,
							fileType: 'image',
							...
							success: (uploadFileRes) => {
								...
								// if (typeof this.uploadSuccess == 'function') {
									...
									if (resData.success) {
										...
										// #endif
										if (this.dataType == 0) {
											this[keyName].push(resData.data)
										} else {
											this[keyName].push({
												type: type,
												url: resData.data,
										...
										this.videoPoster = resData.screenshotName
                                        resolve(uploadFileRes)
										//this.$emit("input", this.uploadLists);
										// #ifndef VUE3
										this.$emit("input", this.uploadLists, resData);
										// #endif
										// #ifdef VUE3
										this.$emit("update:modelValue", this.uploadLists);
										// #endif
...
<style>
	...
	.image-upload-Item-video-app-poster {
		width: 100%;
		height: 100%;
	}
</style>
